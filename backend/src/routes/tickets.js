...
router.get(
  ...
  wrapAsync(async (req, res) => {
    ...
    const payload = await TicketsDBApi.findAll(req.query, { currentUser });
    if (filetype && filetype === 'csv') {
      const fields = ['id', 'title', 'description', 'created_date', 'due_date'];
      const opts = { fields };
      try {
...
router.get(
  ...
  wrapAsync(async (req, res) => {
    const currentUser = req.currentUser;
    const payload = await TicketsDBApi.findAll(req.query, null, {
      countOnly: true,
      currentUser,
    });
